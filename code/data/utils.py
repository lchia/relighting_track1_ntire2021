...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        # for ESTRNN with 4-d data by adding a dimension of the input data
        image = np.ascontiguousarray(image.transpose((2, 0, 1))[np.newaxis, :])
        label = np.ascontiguousarray(label.transpose((2, 0, 1))[np.newaxis, :])

        # for MWCNN with 3-d data
        image = np.squeeze(image, axis=(0,))
        label = np.squeeze(label, axis=(0,))
        #print('__ ToTensor -> image :', image.shape)
        #print('__ ToTensor -> label :', label.shape)
...
def normalize(x, centralize=False, normalize=False, val_range=255.0):
    #print('@_@ normalize -> in  -> x: ', x.shape)
    if centralize:
        x = x - val_range / 2
    if normalize:
        x = x / val_range

    #print('@_@ normalize -> out -> x: ', x.shape)
